...
Prereleases use the form `1.1.0-rc.1` and are published under the `next` dist-tag. Stable releases are published under `latest`.
...
6. **Publish the verified archives.** Publish the archives the suite produced rather than repacking:
   ...
   ```sh
   npm publish release/runtime/neolorn-atlas-1.1.0.tgz
   npm publish release/toolkit/neolorn-atlas-toolkit-1.1.0.tgz
   ```
...
```sh
npm deprecate @neolorn/atlas@1.1.0 "Use 1.1.1"
npm deprecate @neolorn/atlas-toolkit@1.1.0 "Use 1.1.1"
```
